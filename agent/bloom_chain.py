...
SYSTEM_THOUGHT = load_prompt(
    os.path.join(os.path.dirname(__file__), "data/prompts/system/thought.yaml")
)
SYSTEM_RESPONSE = load_prompt(
    os.path.join(os.path.dirname(__file__), "data/prompts/system/response.yaml")
)
SYSTEM_TOOLS = load_prompt(
    os.path.join(os.path.dirname(__file__), "data/prompts/system/tools.yaml")
)
...
class BloomChain:
    def __init__(self, llm: ChatOpenAI, verbose: bool = False):
        ...
        self.system_tools = SystemMessagePromptTemplate(prompt=SYSTEM_TOOLS)
        self.system_response = SystemMessagePromptTemplate(prompt=SYSTEM_RESPONSE)

        # setup tools
        # TODO: setup tool retreival
        self.tools = load_tools(
            ["google-serper", "wolfram-alpha", "wikipedia", "arxiv"], llm=llm
        )
        self.tools_openai = [
            format_tool_to_openai_function(tool) for tool in self.tools
        ]
        self.tool_names = [tool.name for tool in self.tools]
        self.tool_info = [f"{tool.name}: {tool.description}" for tool in self.tools]

    async def think(
        self, thought_memory: ChatMessageHistory, input: str, max_turns: int = 10
    ):
        """Generate Bloom's thought on the user."""
        messages = [
            self.system_thought.format(tools="\n".join(self.tool_info)),
    ...
    async def collect_data(self, thought: str, input: str):
        """Utilize tools to collect data for Bloom's response."""
        messages = [
            self.system_tools.format(thought=thought, tools="\n".join(self.tool_info)),
            HumanMessage(content=input),
        ]

        while True:
            output = await self.llm.apredict_messages(
                messages, functions=self.tools_openai
            )
            print(output.content)
            if "function_call" in output.additional_kwargs:
                tool_name = output.additional_kwargs["function_call"]["name"]
                tool = [tool for tool in self.tools if tool.name == tool_name][0]
                ...
                messages.append(result_message)
                continue
            if output.content.startswith("REFLECTION:"):
                messages.append(output)
                continue
            if output.content.startswith("SUMMARIZE:"):
                return output.content.split("SUMMARIZE:")[1].strip()
    ...
    def respond(self, response_memory: ChatMessageHistory, thought: str, input: str):
        ...
        messages = [
            self.system_response.format(tools=self.tool_names),
